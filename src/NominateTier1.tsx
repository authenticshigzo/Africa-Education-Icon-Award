...
function NominateTier1() {
    ...
    return (
        <main className="flex flex-col items-center w-full min-h-screen mt-[120px] pt-10 sm:mt-[125px] sm:pt-12 pb-12 px-4 sm:px-6 bg-[#0F0E0C] text-white">
            ...
            <p className="mt-3 text-center max-w-2xl text-gray-300">{tierLabels[routeTier]}. Members can nominate a person or organisation in any recognition tier.</p>
            ...
            {!isMember ? (
                ...
                <form onSubmit={handleNominationSubmit} className="flex flex-col gap-4 w-full max-w-xl mt-8 bg-[#2B2824] p-5 sm:p-7 rounded-2xl">
                    ...
                    <select id="tier" value={routeTier} onChange={(event) => handleTierChange(event.target.value as Tier)} className={inputClass}>
